...
static void drawSearchBox(NVGcontext* vg, const char* text, float x, float y, float w, float h) {
	NVGpaint bg;
	char icon[8];
...
static void drawDropDown(NVGcontext* vg, const char* text, float x, float y, float w, float h) {
	NVGpaint bg;
	char icon[8];
...
static void drawEditBoxNum(NVGcontext* vg, const char* text, const char* units, float x, float y, float w, float h) {
	float uw;
...
static void drawCheckBox(NVGcontext* vg, const char* text, float x, float y, float w, float h) {
	NVGpaint bg;
	char icon[8];
	NVG_NOTUSED(w);
...
static void drawThumbnails(NVGcontext* vg, float x, float y, float w, float h, const int* images, int nimages, float t) {
	float cornerRadius = 3.0f;
...
int loadDemoData(NVGcontext* vg, DemoData* data) {
	...
	for (int i = 0; i < 12; i++) {
		char file[128];
		snprintf(file, 128, ASSETS_PREFIX "images/image%d.jpg", i+1);
		data->images[i] = nvgCreateImage(vg, file, 0);
		if (data->images[i] == 0) {
			LOGF("Could not load '%s'.\n", file);
			return -1;
		}
	}

	data->fontIcons = nvgCreateFont(vg, "icons", ASSETS_PREFIX "entypo.ttf");
	if (data->fontIcons == -1) {
		LOGF("Could not add font icons.\n");
		return -1;
	}
	data->fontNormal = nvgCreateFont(vg, "sans", ASSETS_PREFIX "Roboto-Regular.ttf");
	if (data->fontNormal == -1) {
		LOGF("Could not add font italic.\n");
		return -1;
	}
	data->fontBold = nvgCreateFont(vg, "sans-bold", ASSETS_PREFIX "Roboto-Bold.ttf");
	if (data->fontBold == -1) {
		LOGF("Could not add font bold.\n");
		return -1;
	}
	data->fontEmoji = nvgCreateFont(vg, "emoji", ASSETS_PREFIX "NotoEmoji-Regular.ttf");
	if (data->fontEmoji == -1) {
		LOGF("Could not add font emoji.\n");
		return -1;
	}
